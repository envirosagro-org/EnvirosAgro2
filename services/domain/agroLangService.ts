...
import { Plot } from "../ops/spatialService";
...
const ValueBlueprintSchema = z.object({
  blueprint_id: z.string(),
  status: z.enum(["DRAFT", "READY_FOR_ASSETS", "LIVE"]),
  input_material: z.object({
    name: z.string(),
    volume: z.number()
  }),
  value_process_steps: z.array(z.object({
    step_order: z.number(),
    operation: z.string(),
    duration_hours: z.number()
  })),
  asset_requirements: z.array(z.string()),
  projected_value_delta: z.number()
});
...
export const generateValueBlueprint = async (material: string, volume: number): Promise<AgroLangResponse> => {
  try {
    return await callOracleWithRetry(async () => {
      ...
      const validated = ValueBlueprintSchema.parse(parsed);
      return { text: response.text || "", json: validated };
    });
  } catch (err) {
    return handleAIError(err);
  }
