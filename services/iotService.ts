...
import { db, auth, rtdb } from '../services/firebaseService';
...
class IoTSensorService {
  ...
  public async addDroneReading(reading: Omit<DroneTelemetry, 'id' | 'stewardId' | 'timestamp'>) {
    ...
    const data: DroneTelemetry = { ...reading, id: Math.random().toString(36).substr(2, 9), stewardId: userId, timestamp: new Date().toISOString() };
