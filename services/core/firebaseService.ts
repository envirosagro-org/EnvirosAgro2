...
import { notificationService } from "../ops/notificationService";
...
async function testConnection() {
  ...
  try {
    ...
    await getDocFromServer(doc(db, 'system_health', 'connectivity'));
    console.log("Firestore Online Connection: [STABLE]");
  } catch (error) {
...
export const getStewardProfile = async (uid: string): Promise<AgroUser | null> => {
  try {
    ...
    if (snap.exists()) {
      return { ...snap.data(), uid } as AgroUser;
    }
    // If not found, check if it's the guest ESIN
    if (uid === 'EA-GUEST-VOID-NODE') {
      return null; // The caller should handle guest fallback
    }
    return null;
  } catch (e) {
    if (e instanceof Error && (e.message.includes('offline') || e.message.includes('client is offline'))) {
      console.warn(`Steward profile ${uid} fetch skipped: Client is offline.`);
      return null;
    }
