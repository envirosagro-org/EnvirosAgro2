...
import { 
  FileText, Plus, RefreshCw, Loader2, Shield, Info, ExternalLink, CalendarDays, Trash2, Search, ArrowUpRight
} from 'lucide-react';
...
export default function GoogleDocsView() {
  ...
  const fetchDocs = async () => {
    ...
    setLoading(true);
    try {
      const list = await listDocumentsByType('document');
      setDocs(list);
    } catch (err: any) {
      toast.error('Failed to load Google Docs: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setNeedsAuth(false);
        toast.success('Authorized Docs workspace successfully.');
      }
    } catch (e) {
  ...
  const handleCreateDoc = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      const created = await createDocumentByType(newTitle.trim(), 'document');
      toast.success(`Google Doc "${created.name}" created successfully!`);
      setNewTitle('');
      setShowForm(false);
      fetchDocs(); // reload list
    } catch (err: any) {
      toast.error('Failed to create Google Doc: ' + err.message);
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDeleteDoc = async (id: string, name: string) => {
    const confirmed = window.confirm(`Trash "${name}" document from Google Drive?`);
    if (!confirmed) return;
    ...
    <div className="p-6 md:p-12 space-y-10 animate-in fade-in duration-700 max-w-[1400px] mx-auto">
      ...
      <div className="glass-card p-12 rounded-[56px] border border-blue-500/20 bg-blue-500/5 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-12 shadow-3xl">
         <div className="absolute top-0 right-0 p-12 opacity-[0.05] pointer-events-none rotate-12">
            <FileText className="w-96 h-96 text-blue-400" />
         </div>
         <div className="flex items-center gap-10 relative z-10 text-center md:text-left flex-col md:flex-row">
            ...
            <div className="space-y-4">
               <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter italic m-0 leading-none drop-shadow-2xl">
                 Google <span className="text-blue-400">Docs</span> Portal
               </h2>
               <p className="text-slate-400 text-lg font-medium italic leading-relaxed max-w-2xl">
                 Author agronomic papers, training guides, and organic seed standards directly utilizing Google Docs cloud workspaces.
               </p>
            </div>
         </div>
      ...
      {needsAuth ? (
        <div className="glass-card p-16 rounded-[48px] border border-white/5 bg-black/40 text-center space-y-8 max-w-2xl mx-auto my-20">
           ...
           <div className="space-y-4">
              <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter">Authorized Documents</h3>
              <p className="text-slate-400 italic text-base leading-relaxed">
                Connect your account to browse, read, and write cloud-backed Google Docs directly compiled under the local agricultural standards panel.
              </p>
           </div>
           <button 
             onClick={handleLogin}
        ...
        <div className="space-y-12">
          ...
          <div className="max-w-3xl mx-auto relative z-10">
             <div className="glass-card p-8 rounded-[36px] border border-white/5 bg-[#050706]">
                ...
                >
                   <span>Create New Document Shard</span>
                   <Plus size={18} className={`text-blue-400 transition-transform ${showForm ? 'rotate-45' : ''}`} />
                </button>
                ...
                <AnimatePresence>
                   {showForm && (
                      ...
                      >
                         <label className="text-[10px] text-slate-500 font-black uppercase tracking-wider block">Document Title</label>
                         <div className="flex gap-4">
                            <input 
                              placeholder="e.g. Kinshasa Organic Soil Diagnostics Standard" 
                              value={newTitle} 
                              onChange={e => setNewTitle(e.target.value)}
                              className="flex-1 bg-white/5 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-blue-500 transition-all font-medium"
                            />
                            <button 
                              type="submit" 
                              disabled={isCreating}
                              className="px-8 bg-blue-600 hover:bg-blue-500 rounded-xl text-white font-black text-[10px] uppercase tracking-wider flex items-center gap-2 active:scale-95 transition-all shadow-xl disabled:opacity-45"
                            >
          ...
          <div className="space-y-8">
             <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-white/5">
                <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter">Your <span className="text-blue-400">Google Docs</span></h3>
                <div className="relative w-full md:w-80">
                   ...
                   <input 
                     ...
                     className="w-full bg-white/5 border border-white/10 rounded-full pl-12 pr-6 py-3 text-white text-xs outline-none focus:border-blue-500 transition-all placeholder-slate-700 font-sans"
                   />
                </div>
             </div>
             ...
             {loading ? (
                <div className="py-24 text-center space-y-6">
                   ...
                   <p className="text-[11px] font-black uppercase text-slate-600 tracking-widest">Scanning cloud document registries...</p>
                </div>
             ) : filteredDocs.length === 0 ? (
                <div className="py-24 text-center opacity-30 space-y-6">
                   <FileText size={64} className="text-slate-600 mx-auto animate-pulse" />
                   <p className="text-xs font-black uppercase text-slate-500 tracking-wider">No Google documents match query.</p>
                </div>
             ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                   {filteredDocs.map(doc => (
                      <div key={doc.id} className="p-8 bg-[#050706] rounded-3xl border border-white/5 flex flex-col justify-between min-h-[200px] group hover:border-blue-500/20 transition-all">
                         <div className="space-y-4">
                            <div className="flex justify-between items-start">
                               <div className="p-3 bg-blue-500/10 rounded-2xl text-blue-400"><FileText size={20} /></div>
                               <button 
                            ...
                            <h4 className="text-lg font-black text-white uppercase italic tracking-tight line-clamp-2 leading-tight">{doc.name}</h4>
                         </div>
                         ...
                         <div className="flex justify-between items-center pt-6 border-t border-white/5 mt-6">
                            <span className="text-[9px] font-mono text-slate-600 font-bold uppercase">
                               Created {doc.createdTime ? new Date(doc.createdTime).toLocaleDateString() : 'N/A'}
                            </span>
                            {doc.webViewLink && (
                               <a 
                                 ...
                                 target="_blank" 
                                 rel="noreferrer" 
                                 className="px-5 py-2.5 bg-white/5 hover:bg-blue-600 border border-white/10 text-white rounded-xl text-[9px] font-black uppercase tracking-wider flex items-center gap-1.5 transition-all shadow active:scale-95"
                               >
                                  Open Doc
