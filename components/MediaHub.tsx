...
const MediaHub: React.FC<MediaHubProps> = ({ user, userBalance, onSpendEAC, onEarnEAC, onNavigate, initialSection, initialAction }) => {
  const [tab, setTab] = useState<'all' | 'video' | 'news' | 'audio' | 'streaming' | 'comic'>('all');
  ...
  useEffect(() => {
    if (initialAction) {
      ...
      if (['all', 'video', 'news', 'audio', 'streaming', 'comic'].includes(initialSection)) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-7xl mx-auto px-1 md:px-4">
      ...
      <div className="min-h-fit">
              ...
              {loadingNews ? (
                <div className="py-40 flex flex-col items-center justify-center space-y-10">
                   <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center"><HenIcon size={40} className="text-emerald-400 animate-pulse" /></div>
                   </div>
        ...
        {/* TAB: MAKE IT COMIC */}
        {tab === 'comic' && (
          <div className="space-y-12 animate-in slide-in-from-bottom-10 duration-700 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="glass-card p-12 rounded-[64px] border-2 border-white/5 hover:border-indigo-500/40 transition-all group flex flex-col justify-between min-h-[400px] bg-black/40 shadow-3xl relative overflow-hidden active:scale-[0.99] cursor-pointer" onClick={() => onNavigate('multimedia_generator' as any, null as any, true, { prompt: 'Create a funny stand-up comedy routine about the struggles of a modern farmer dealing with unpredictable weather and stubborn tractors.', type: 'audio', autoGenerate: true })}>
                ...
                <div className="pt-8 border-t border-white/5 flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white"><HenIcon size={14} /></div>
              ...
              <div className="glass-card p-12 rounded-[64px] border-2 border-white/5 hover:border-emerald-500/40 transition-all group flex flex-col justify-between min-h-[400px] bg-black/40 shadow-3xl relative overflow-hidden active:scale-[0.99] cursor-pointer" onClick={() => onNavigate('multimedia_generator' as any, null as any, true, { prompt: 'Create an action-packed, fast-paced sports highlight reel of a competitive tractor pulling event, with dramatic angles and intense music.', type: 'video', autoGenerate: true })}>
                ...
                <div className="pt-8 border-t border-white/5 flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white"><Video size={14} /></div>
