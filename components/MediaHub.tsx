...
import { 
  ...
  AudioWaveform,
  Podcast,
  // Added PencilRuler to fix 'Cannot find name' error on line 598
  PencilRuler
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-7xl mx-auto px-1 md:px-4">
                ...
                <div className="glass-card p-10 rounded-[56px] border border-emerald-500/20 bg-black/40 space-y-8 shadow-2xl relative overflow-hidden group">
                   ...
                   <div className="flex items-center gap-6 relative z-10">
                      ...
                      <div>
                         ...
                         <p className="text-[10px] font-mono text-emerald-500/60 font-bold uppercase tracking-widest mt-2">DEVICE_SYNC: {isBroadcasting ? 'ACTIVE_LENS' : 'STANDBY'}</p>
                      </div>
                   </div>
                   <div className="space-y-6 relative z-10">
                      <div className="space-y-3">
                         <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest px-4">Broadcast Modality</label>
                         <div className="grid grid-cols-2 gap-3">
                            {[
                               ...
                               <button 
                                 ...
                                 className={`p-4 rounded-[28px] border-2 transition-all flex flex-col items-center gap-2 group/btn ${streamMode === m.id ? 'bg-emerald-600/10 border-emerald-500 text-white shadow-xl' : 'bg-black border-white/10 text-slate-500 hover:border-white/30'}`}
                               >
                                  <m.i size={18} className={streamMode === m.id ? 'text-emerald-400' : 'text-slate-700 group-hover/btn:text-slate-400'} />
                   ...
                   <div className="pt-4 border-t border-white/5 relative z-10">
                      {isBroadcasting ? (
                        ...
                        <button onClick={handleStartStream} disabled={isInitializing} className="w-full py-8 agro-gradient rounded-[40px] text-white font-black text-xs uppercase tracking-[0.5em] shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4">
                           ...
                           {isInitializing ? 'CALIBRATING...' : 'INITIALIZE DIRECT LINK'}
                        </button>
                      )}
                   </div>
                </div>
                <div className="p-8 glass-card rounded-[48px] border border-indigo-500/10 bg-indigo-500/5 space-y-6">
                   <div className="flex items-center gap-3">
                      <ShieldCheck className="w-5 h-5 text-indigo-400" />
                      <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest italic">Verification Protocol</h4>
                   </div>
                   <div className="grid grid-cols-2 gap-4">
                      <button onClick={() => setShowSignatureModal('video')} disabled={!isBroadcasting} className="py-4 bg-white/5 border border-white/10 rounded-2xl text-[9px] font-black uppercase text-slate-400 hover:text-white transition-all flex flex-col items-center gap-2 disabled:opacity-20">
                         <Signature size={18} /> Video Signature
                      </button>
                      <button onClick={() => setShowSignatureModal('voice')} disabled={!isBroadcasting} className="py-4 bg-white/5 border border-white/10 rounded-2xl text-[9px] font-black uppercase text-slate-400 hover:text-white transition-all flex flex-col items-center gap-2 disabled:opacity-20">
                         <Mic2 size={18} /> Voice Signature
                      </button>
                   </div>
                </div>
             </div>
             <div className="lg:col-span-8 flex flex-col space-y-8">
                <div className="glass-card rounded-[64px] border-2 border-white/5 bg-black overflow-hidden relative group min-h-[600px] shadow-3xl">
                   ...
                   {!isBroadcasting && (
                     <div className="absolute inset-0 flex flex-col items-center justify-center space-y-12 py-40 opacity-20">
                        ...
                        <div className="text-center space-y-2">
                           ...
                           <p className="text-lg font-bold text-slate-600 uppercase tracking-widest italic">Allow Camera/Audio access to begin sharding</p>
                        </div>
                     </div>
                   )}
                   {isBroadcasting && (
                     <>
                        <div className="absolute top-10 left-10 flex gap-4">
                           <div className="px-6 py-2 bg-rose-600 rounded-full text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-2xl animate-pulse">
                              <div className="w-2 h-2 rounded-full bg-white animate-ping"></div> REGISTRY LIVE
                           </div>
                           <div className="px-6 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full text-white text-[10px] font-black uppercase tracking-widest">MODE: {streamMode}</div>
                        </div>
                        <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end">
                           <div className="p-8 bg-black/60 backdrop-blur-xl border border-white/10 rounded-[40px] max-w-sm space-y-4">
                              <div className="flex items-center gap-3">
                                 ...
                                 <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{oracleActive ? 'Oracle Connected' : 'Oracle Monitoring...'}</span>
                              </div>
                              <p className="text-sm text-slate-300 italic font-medium leading-relaxed">
                                 {streamMode === 'ORACLE_PODCAST' ? '"Oracle listening for biological voice signature..."' : '"Telemetry shards anchoring to registry from direct lens ingest..."'}
                              </p>
                           </div>
                        </div>
                     </>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="glass-card p-10 rounded-[56px] border border-white/5 bg-black/40 space-y-8 shadow-xl">
                      ...
                      <div className="space-y-4 max-h-[200px] overflow-y-auto custom-scrollbar font-mono text-[10px]">
                         {isBroadcasting ? (
                            <><p className="text-indigo-400">[SYSTEM] DIRECT_LENS_HANDSHAKE_SUCCESS</p><p className="text-slate-500">[INGEST] BUFFER_POOL_STABLE_{bitrate}kbps</p><p className="text-emerald-500">[AUTH] REGISTRY_ANCHOR_SUCCESS</p></>
                         ) : (<p className="text-slate-700 italic text-center py-10 uppercase tracking-widest">Awaiting Signal...</p>)}
                      </div>
                   </div>
                   <div className="glass-card p-10 rounded-[56px] border border-emerald-500/20 bg-emerald-500/5 flex flex-col justify-center text-center shadow-xl">
                      <p className="text-[10px] text-emerald-500 font-black uppercase tracking-[0.4em] mb-2">Verified Yield Shards</p>
                      <h4 className="text-6xl font-mono font-black text-white tracking-tighter">{isBroadcasting ? Math.floor(streamDuration / 10) : '0'}</h4>
                   </div>
                </div>
             </div>
          </div>
        ...
        {tab === 'waves' && (
          <div className="max-w-5xl mx-auto space-y-12 animate-in zoom-in duration-500">
             <div className="p-16 glass-card rounded-[80px] border border-emerald-500/20 bg-emerald-950/5 relative overflow-hidden flex flex-col items-center text-center space-y-12 shadow-3xl group">
                ...
                <button onClick={() => setShowAudioUpload(true)} className="px-16 py-8 agro-gradient rounded-full text-white font-black text-sm uppercase tracking-[0.5em] shadow-2xl hover:scale-105 active:scale-95 transition-all relative z-10 flex items-center gap-4 border-2 border-white/10 group">
                   <PlusCircle className="w-6 h-6 group-hover:rotate-90 transition-transform" /> INGEST BIO-SIGNAL
                </button>
             </div>
          </div>
        )}
      ...
      {showSignatureModal && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center p-4">
           ...
           <div className="relative z-[710] w-full max-w-xl glass-card p-12 rounded-[56px] border border-white/10 bg-[#050706] shadow-3xl text-center space-y-10 border-2">
      ...
      {activeAudioTrack && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[800] w-full max-w-4xl px-4 animate-in slide-in-from-bottom-10 duration-700">
           <div className="glass-card p-8 rounded-[40px] border-emerald-500/30 bg-[#050706]/95 backdrop-blur-2xl shadow-[0_0_100px_rgba(0,0,0,0.9)] flex items-center gap-10 border-2">
              <div className="w-20 h-20 bg-emerald-600 rounded-3xl flex items-center justify-center text-white shrink-0 shadow-2xl"><AudioWaveform size={40} className="animate-pulse" /></div>
              <div className="flex-1 min-w-0">
                 <h4 className="text-xl font-black text-white uppercase italic truncate m-0">{activeAudioTrack.title}</h4>
                 <div className="flex items-center gap-6 mt-3">
                    <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">{activeAudioTrack.type} // {activeAudioTrack.duration}</span>
                    <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden relative shadow-inner">
                       <div className="h-full bg-emerald-500 rounded-full animate-pulse" style={{ width: '42%' }}></div>
                    </div>
                 </div>
              </div>
              <div className="flex items-center gap-4 shrink-0">
                 <button className="p-4 bg-white/5 rounded-2xl text-slate-500 hover:text-white transition-all"><History size={20} /></button>
                 <button onClick={() => setIsPlaying(!isPlaying)} className="w-20 h-20 bg-emerald-600 rounded-full flex items-center justify-center text-white shadow-3xl hover:bg-emerald-500 active:scale-90 transition-all border-4 border-white/5">
                    {isPlaying ? <Pause size={32} className="fill-current" /> : <Play size={32} className="fill-current translate-x-1" />}
                 </button>
                 <button onClick={() => setActiveAudioTrack(null)} className="p-4 bg-white/5 rounded-2xl text-slate-500 hover:text-rose-500 transition-all"><X size={20} /></button>
              </div>
           </div>
        </div>
      )}
