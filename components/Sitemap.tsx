    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-24 max-w-[1600px] mx-auto px-4">
      ...
      <div className="glass-card p-12 md:p-20 rounded-[80px] border-emerald-500/20 bg-emerald-500/5 relative overflow-hidden flex flex-col items-center text-center space-y-10 shadow-3xl group">
         ...
         <div className="space-y-6 relative z-10">
            <h2 className="text-5xl md:text-8xl font-black text-white uppercase italic m-0 drop-shadow-2xl">REGISTRY <span className="text-emerald-400">MATRIX</span></h2>
            <p className="text-slate-400 text-2xl font-medium italic leading-relaxed max-w-3xl mx-auto">
               "Navigating the multi-thrust architecture of EnvirosAgro. Every shard, protocol, and node addressable at the atomic level via Vector Routing."
            </p>
         </div>
         
         <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-4xl relative z-10 pt-12 border-t border-white/5">
            {[
              { l: 'Dimensions (D)', v: nodes.length.toString(), i: Layers },
              { l: 'Total Elements (E)', v: nodes.reduce((acc, g) => acc + g.items.length, 0).toString(), i: Database },
              { l: 'Routing Protocol', v: 'VECTOR_V1', i: Workflow },
              { l: 'Network Sync', v: 'STABLE', i: Activity }
            ].map((s, idx) => (
      ...
      {/* Synchronized Book Matrix */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
        {nodes.map((group, dIdx) => (
          <div key={dIdx} className="glass-card p-12 rounded-[72px] border-2 border-white/5 bg-black/60 space-y-10 shadow-2xl relative group/card overflow-hidden h-fit transition-all duration-500 hover:border-emerald-500/20">
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-8">
               <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-700 font-mono font-black uppercase tracking-widest italic">MATRIX_DIMENSION_{dIdx + 1}</p>
               </div>
               <div className="p-4 bg-emerald-500/10 rounded-2xl text-emerald-500 border border-emerald-500/20 shadow-xl font-mono font-black">D{dIdx + 1}</div>
            </div>

            <div className="grid gap-10">
               {group.items.map((item, eIdx) => (
                  <div key={item.id} className="space-y-6">
                    {/* Primary Node Link with Address Tag */}
                    <button 
                      onClick={() => onNavigate(item.id as ViewState)}
                      className="w-full flex items-center justify-between p-6 bg-white/[0.02] hover:bg-emerald-600/10 border border-white/5 hover:border-emerald-500/30 rounded-[40px] transition-all group/node text-left shadow-inner relative overflow-hidden"
                    >
                       <div className="flex items-center gap-6 relative z-10">
                          ...
                          <div className="space-y-2">
                             <div className="flex items-center gap-3">
                                <p className="text-xl font-black text-slate-200 uppercase italic leading-none group-hover:text-emerald-400 transition-colors m-0">{item.name}</p>
                                <span className="text-[9px] font-mono text-emerald-500/40 bg-emerald-500/5 px-2 py-0.5 rounded border border-emerald-500/10">[{dIdx+1}.{eIdx+1}]</span>
                             </div>
                             <p className="text-[10px] text-slate-700 font-mono font-bold uppercase tracking-widest mt-1">#/{item.id}</p>
                          </div>
                       </div>
                       <ArrowRight size={20} className="text-slate-800 opacity-0 group-hover/node:opacity-100 group-hover/node:translate-x-1 transition-all relative z-10" />
                       <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/[0.01] to-transparent pointer-events-none"></div>
                    </button>

                    {/* Element Section Shards (Vectors) */}
                      ...
                      <div className="pl-8 flex flex-wrap gap-3 animate-in fade-in duration-1000">
                        {item.sections.map((section, sIdx) => (
                          <button
                            ...
                            title={section.desc}
                          >
                             <span className="text-[8px] font-mono text-indigo-500/40 mr-1">[{dIdx+1}.{eIdx+1}.{sIdx+1}]</span>
      ...
      <div className="p-16 md:p-24 glass-card rounded-[80px] border-indigo-500/30 bg-indigo-950/5 flex flex-col md:flex-row items-center justify-between gap-16 relative overflow-hidden shadow-3xl">
         ...
         <div className="flex items-center gap-12 relative z-10 text-center md:text-left flex-col md:flex-row">
            ...
            <div className="space-y-6">
               <h4 className="text-4xl md:text-5xl font-black text-white uppercase italic m-0 leading-none">TOTAL <span className="text-indigo-400">SYNCHRONIZATION</span></h4>
               <p className="text-slate-400 text-2xl md:text-3xl font-medium italic leading-relaxed max-w-2xl opacity-80">
                 "Direct sharding of every dimension and element. The registry sitemap serves as the immutable book of the network topology."
               </p>
            </div>
         </div>
         <div className="text-center md:text-right shrink-0 px-12 relative z-10 border-l-2 border-white/5 hidden lg:block">
            <p className="text-[12px] text-slate-600 font-black uppercase mb-6 tracking-[0.8em]">ADDRESSABLE_SHARDS</p>
