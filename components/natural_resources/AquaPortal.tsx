...
const AquaPortal: React.FC<AquaPortalProps> = (props) => {
  ...
  const meta: ResourceMeta = {
    ...
    simControls: [
      { label: 'm: Resilience', val: ecosystemState.p1, set: (v: number) => updateEcosystemState({ p1: v }), min: 1, max: 2, step: 0.01 },
      { label: 'V: Storage', val: ecosystemState.p2, set: (v: number) => updateEcosystemState({ p2: v }), min: 100, max: 5000, step: 10 },
      { label: 'D: Loss', val: ecosystemState.p3, set: (v: number) => updateEcosystemState({ p3: v }), min: 1, max: 100, step: 1 },
    ],
    ledgerItems: [
