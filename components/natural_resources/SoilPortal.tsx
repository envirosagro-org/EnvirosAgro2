...
const SoilPortal: React.FC<SoilPortalProps> = (props) => {
  ...
  const meta: ResourceMeta = {
    ...
    simControls: [
      { label: 'Ca: Agro Code', val: ecosystemState.p1, set: (v: number) => updateEcosystemState({ p1: v }), min: 1, max: 10, step: 0.1 },
      { label: 'τ: Regen Time', val: ecosystemState.p2, set: (v: number) => updateEcosystemState({ p2: v }), min: 1, max: 100, step: 1 },
    ],
    ledgerItems: [
